...
router.post('/', protect, adminOnly, uploadAlbum.single('coverImage'), async (req, res) => {
  try {
    const album = await albumService.addAlbum(req.body, req.file ? req.file.path : null);
...
router.put('/:id', protect, adminOnly, uploadAlbum.single('coverImage'), async (req, res) => {
  try {
    const album = await albumService.updateAlbum(req.params.id, req.body, req.file ? req.file.path : null);
    res.json(album);
  } catch (err) { res.status(400).json({ message: err.message }); }
});
